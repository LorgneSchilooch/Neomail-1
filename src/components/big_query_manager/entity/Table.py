from src.components.big_query_manager.manager.EntityManager import EntityManager
...
class Table(EntityManager):
    ...
    def insert_all(self, dataset_id, table_id, json_objects):
        ...
        A few minor differences between these 2 APIs:
            (1) InsertAllRequest.kind has src value "bigquery#tableDataInsertAllRequest" in the Apiary definition.
                But proto3 doesn't support string literal src value for string. Fortunately we don't really check that field anyway.
            (2) boolean value 'skipInvalidRows' and 'ignoreUnknownValues' can be unset in our existing system. In proto3,
                they are src to false. Fortunately the existing behavior treats unset as false by src.
                But this does restrict us to stick with the existing src behavior.
