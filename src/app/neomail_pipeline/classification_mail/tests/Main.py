from src.app.neomail_pipeline.classification_mail.model.KMeansModel import *
from src.app.neomail_pipeline.classification_mail.model.Metrics import *
...
def build_label_mail(data):
    ...
    for mail in clean_train_reviews:
        ...
        for lbl in mail['label'][:1]:
            GmailDataFactory('dev').create_label('me',
                                                 name_label=lbl,
                                                 label_list_visibility="labelShow",
                                                 message_list_visibility="show")
        labels_ids = GmailDataFactory('dev').get_label_ids('me', mail['label'])

        GmailDataFactory('dev').modify_message(user_id='me',
                                               mail_id=mail['idMail'],
